        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <dl class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Reported</dt>
                        <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $report->created_at->format('M d, Y g:i A') }}</dd>
                    </div>
                    <div class="col-span-2">
                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Reason</dt>
                        <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $report->reason }}</dd>
                    </div>
                </dl>
            </div>
            ...
            @if($report->status === 'pending')
                <form action="{{ route('admin.moderation.resolve', $report) }}" method="POST" class="border-t dark:border-gray-700 pt-6">
                    ...
                    <div class="flex justify-end space-x-4">
                        <a href="{{ route('admin.moderation.index') }}" 
                           class="px-6 py-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition">
                            Cancel
                        </a>
                        <button type="submit" 
            ...
            @else
                <div class="border-t dark:border-gray-700 pt-6">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Resolution</h3>
                    <dl class="grid grid-cols-2 gap-4">
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Status</dt>
                            <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ ucfirst($report->status) }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Moderator</dt>
                            <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $report->moderator->name ?? 'N/A' }}</dd>
                        </div>
                        @if($report->moderator_notes)
                            <div class="col-span-2">
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Notes</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $report->moderator_notes }}</dd>
                            </div>
                        @endif
                    </dl>
                </div>
