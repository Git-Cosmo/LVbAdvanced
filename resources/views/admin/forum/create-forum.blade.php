...
<div class="max-w-3xl mx-auto">
    <div class="mb-6">
        <h1 class="text-3xl font-bold dark:text-dark-text-bright text-light-text-bright">Create Forum</h1>
    </div>
    
    <div class="dark:bg-dark-bg-secondary bg-light-bg-secondary rounded-xl p-6">
        <form action="{{ route('admin.forum.forum.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-6">
                ...
                <select id="parent_id" 
                    ...
                    <option value="{{ $forum->id }}" {{ old('parent_id') == $forum->id ? 'selected' : '' }}>
                        {{ $forum->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="mb-6">
                ...
                <textarea id="description" 
                          name="description" 
                          rows="3"
                          class="w-full px-4 py-3 dark:bg-dark-bg-tertiary bg-light-bg-tertiary dark:text-dark-text-primary text-light-text-primary rounded-lg focus:outline-none focus:ring-2 focus:ring-accent-blue">{{ old('description') }}</textarea>
            </div>
